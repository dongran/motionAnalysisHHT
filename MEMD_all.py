...
def envelope_mean(m,t,seq,ndir,N,N_dim): #new
    ...
    for it in range(0,ndir):
        ...
        if mode:
            ...
            # env_max = np.zeros((N, N_dim))
            # for i in range(N_dim):
            #     f_Lag=lagrange(tmax,zmax[:,i])
            #     env_max[:,i] = f_Lag(t)
...
#window
    ...
    inf_WAFA=np.zeros((N,num))
    
    for i in range (0, num):
    #for k=1:(m-1)/2
        for k in range(0,int((m-1)/2)):
            b = 0
            e=int(k+(m-1)/2)
            inf_WAFA[k,i]=np.sum(inf[b:e,i]*amp[b:e,i])/(np.sum(amp[b:e,i]))
            
    
        #for  k=(m+1)/2:(2*N-1-m)/2
        for  k in range(int((m+1)/2)-1, int((2*N-1-m)/2)):
             b=int(k-(m-1)/2)
             e=int(k+(m-1)/2)
             inf_WAFA[k,i]=np.sum(inf[b:e,i]*amp[b:e,i])/(np.sum(amp[b:e,i]))
        ...
        e=N;
    #for  k=(2*N+1-m)/2:N
        for  k in range(int((2*N+1-m)/2)-1, N):    
            b=int(k-(m-1)/2); 
            inf_WAFA[k,i]=np.sum(inf[b:e,i]*amp[b:e,i])/(np.sum(amp[b:e,i]))
...
def avgF2(avgf):
    ...
    avgf2 = np.empty([0, num])
     
    #tmp = np.zeros([1,num])
    
    
    for i in range (int(joints/3)):
        
        tmp=np.sqrt(np.power(avgf[:,i*3],2)+np.power(avgf[:,(i*3)+1],2)+np.power(avgf[:,(i*3)+2],2))
        
        if str(tmp[0]) != 'nan':
               avgf2 = np.r_[avgf2,tmp.reshape([1,num])]
...
def avgFibo(avgf):
    ...
    for _ in range (num):
        ...
        tmp = avgf[i-2]+avgf[i-1]
        avgf2[i-3] = np.abs(1-(avgf[i-3]/tmp))
        i=i-1
        
        
    return(np.round(avgf2,decimals=2))
...
def avgA(IMF,dt):
    ...
    for i in range (joints):
        freq, amp = hs.FAhilbert(IMF[:,i,:], dt)
        for j in range (num):
            tmp[0,j]=np.mean(amp[:,j])
        # if str(tmp[0,0]) != 'nan':
        #     avgf = np.r_[avgf,tmp]
        tmp2=(tmp-np.min(tmp))/(np.max(tmp)-np.min(tmp))
        avga = np.r_[avga,tmp2]
...
#     for j in range(m):            
#         plt.scatter(t[:,j], inf[:,j], s=num, c=amp[:,j], cmap='jet')
